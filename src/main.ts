...
import { globalCorsOrigins } from './shared/config/global-db-config';
...
async function bootstrap() {
  ...
  const crateDbService = app.get(CrateDbService);

  let query = "SELECT CONCAT('http://', domains, ':5600') AS origin  FROM dev_meta.application";
  const result = await crateDbService.executeQuery(query);  

  const corsOrigins = result.data.map((row) => row.origin);    

  corsOrigins.forEach(element => {
    globalCorsOrigins.push(element)
});
  app.enableCors({
    origin: globalCorsOrigins,  
    // Other CORS configuration options if needed
  });
  ...
  //     , 'http://cloud.com:5600','http://clouds.com:5600' , 'http://governance.expocitydubai.com:5600' , 'http://accounts.abc.com:5600' , 'http://jaleel.com:5600', 'http://pakapp1.com:5600' 
